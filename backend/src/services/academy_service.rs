...
pub async fn get_course_full(
    pool: &PgPool,
    course_id: Uuid,
) -> Result<CourseFullDto, AppError> {


    let course = get_course_by_id(pool, course_id)
        .await?;


    let rows = academy_repo::get_course_full_rows(
        pool,
        course_id,
    )
    .await?;
    ...
    let mut sections: HashMap<Uuid, SectionDto> =
        HashMap::new();
    ...
    let mut sections: Vec<SectionDto> =
...
pub async fn update_course(
    ...
    let course = academy_repo::update_course(
        pool,
        id,
        req,
    )
...
pub async fn get_sections_by_course(
    ...
    Ok(
        academy_repo::get_sections_by_course(
            pool,
            course_id,
        )
        .await?
...
pub async fn update_section(
    ...
    let section = academy_repo::update_section(
        pool,
        id,
        req,
    )
...
pub async fn get_tasks_by_section(
    ...
    Ok(
        academy_repo::get_tasks_by_section(
            pool,
            section_id,
        )
        .await?
