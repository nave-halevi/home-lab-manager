...
pub async fn get_flag(pool: &PgPool, scenario_id: Uuid, flag_value: &str) -> Result<Option< (Uuid, i32)> , Error> {
    let record = sqlx::query!(
...
pub async fn submit_and_score(pool: &PgPool, user_id: Uuid, flag_id: Uuid, points: i32) -> Result<(), Error> {
